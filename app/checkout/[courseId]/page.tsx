...
export default async function CheckoutPage({ params }: CheckoutPageProps) {
  ...
  const { data: batchRows, error: batchError } = await supabase
    .from("batches")
    .select("id, batch_name, description, start_date")
    .eq("course_id", course.id)
    .eq("is_active", true)
    .order("start_date", { ascending: true });

  const batches =
    !batchError && batchRows
      ? batchRows.map((batch) => ({
          id: batch.id,
      ...
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-4xl border border-ink/10 bg-white p-8 shadow-glow">
          ...
          <p className="mt-4 text-base leading-8 text-slate">
            Confirm the published course details and choose an active batch before continuing to
            Razorpay payment.
          </p>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 font-heading text-3xl font-bold text-ink">
                  Select an active batch from Supabase
                </h2>
              </div>
              <div className="rounded-3xl bg-cream px-4 py-3 text-right">
                <p className="text-sm text-slate">Available batches</p>
                <p className="font-heading text-3xl font-bold text-ink">{batches.length}</p>
              </div>
            </div>
            <div className="mt-6 grid gap-4">
              {batches.map((batch) => (
                <article key={batch.id} className="rounded-4xl border border-ink/10 bg-cream p-6">
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.16em] text-coral">
                        Batch Name
                      </p>
                      <h3 className="mt-2 font-heading text-2xl font-bold text-ink">{batch.name}</h3>
                    </div>
                    <div className="grid gap-4 sm:grid-cols-2 lg:min-w-[22rem]">
                      ...
                      <div className="rounded-3xl bg-white px-4 py-3">
                        ...
                        <p className="mt-1 font-semibold text-ink">{batch.teacher}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-5 rounded-3xl bg-white px-4 py-4">
                    <p className="text-sm text-slate">Batch notes</p>
                    <p className="mt-2 text-base leading-7 text-ink">
                      {batch.description || "No additional batch description has been published yet."}
                    </p>
                  </div>
                </article>
              ))}
        ...
        <aside className="rounded-4xl bg-ink p-8 text-cream shadow-glow">
          ...
          <div className="mt-8">
            {batches.length > 0 ? (
              <PaymentButton courseId={course.id} batches={batches} />
            ) : (
              <div className="rounded-3xl border border-coral/20 bg-coral/10 px-5 py-4 text-sm text-coral">
                No active batches are available for this course right now. Please contact support or
                check back soon.
              </div>
            )}
          </div>
